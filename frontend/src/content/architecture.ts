...
export const ARCH_TABS: ArchTab[] = [
  {
    ...
    body: {
      ...
      es: [
        'Rajo es un observatorio público de los rajos y las pozas de litio del mundo: treinta sitios (doce en Chile), cada uno una ventana de 12 a 40 km sobre su propia grilla UTM, vistos cada año desde 1985 hasta hoy (Landsat 5, 7, 8 y 9 hasta 2016, Sentinel-2 desde 2017) y calculados en el navegador desde archivos abiertos. Nada se sube y no hay backend.',
        'Ciclo de diseño y construcción: investigación primero (cada fuente de datos probada por CORS, lecturas por rango y licencia; treinta y cinco artículos con DOI resuelto; el plan persistido en el repositorio de gestion), luego los dos contratos de datos (ingesta: el catálogo de sitios validado contra los polígonos de referencia; artefacto: los manifiestos con bytes y sha256 de cada archivo), luego la cocina offline por etapas (catalog, scenes, frames, masks, series, dense, dem, export, validate) corrida desacoplada y en paralelo, luego el entrenamiento de los modelos aprendidos, luego la SPA (React 19 + Vite, MapLibre GL, uPlot, onnxruntime-web, geotiff.js), luego un despliegue estático con una verificación de contenido en vivo.',
        'Los artefactos versionados bajo data/derived y los modelos ONNX bajo models/ son el producto; la app web los reproduce y agrega tres carriles genuinamente en vivo: lecturas Sentinel-2 desde los GeoTIFF optimizados para la nube, los cálculos espectrales y clásicos en un Web Worker, y los modelos aprendidos sobre el proveedor WebGPU o WASM.',
      ],
    },
  },
  {
    ...
    body: {
      ...
      es: [
        'En vivo en la web: el instrumento lee el grupo Sentinel-2 del mismo dia más reciente y despejado directamente del bucket sentinel-cogs (peticiones HTTP por rango con geotiff.js, un pool de decodificación, una señal de aborto) sobre la grilla del sitio; el worker de matemática de bandas (indices.worker.ts) calcula compuestos, nueve índices, Otsu, k-means y el ángulo espectral; el mismo worker construye los dieciséis planos de atributos, recorre el bosque aleatorio desde arreglos planos de nodos (forest.ts, porque onnxruntime-web no tiene kernel para ensambles de árboles) y corre la U-Net con onnxruntime-web (webgpu cuando la página lo tiene, wasm si no); el cajón de series reejecuta PELT en TypeScript; la herramienta de perfil decodifica teselas terrarium de ambas épocas.',
        'Offline (la cocina, un venv Python 3.12, scripts planos por ruta): la etapa catalog valida los sitios y recorta los polígonos de referencia; scenes busca en Earth Search y el Planetary Computer por sitio y año; frames reproyecta las bandas a la grilla (rasterio WarpedVRT), compone los vacíos de Landsat 7, renderiza cuadros WebP y guarda los chips; masks evalúa cada cuadro con Otsu, el bosque y la U-Net dentro de la envolvente; series construye la serie de área minada y sus puntos de cambio; dense lee cada fecha Sentinel-2 despejada desde 2017 a 60 m; dem diferencia SRTM y el DEM Copernicus y cocina las teselas de terreno; export escribe los manifiestos y validate rechaza un hueco.',
        'Cómputo (el carril GPU): fetch_tiles.py trae las 1.207 teselas de entrenamiento de Jasansky et al. 2024 desde Earth Search, train_rf.py ajusta el bosque y lo exporta con una compuerta de paridad scikit-learn contra onnxruntime, train_unet.py entrena la U-Net desde un banco de recortes mapeado en memoria y export_unet.py entrega fp32 y fp16 con una compuerta de paridad PyTorch contra onnxruntime; evaluate.py evalúa cada método sobre las mismas teselas retenidas. La reproducción es el respaldo común: la web carga los artefactos versionados y nunca recomputa la cocina.',
      ],
    },
  },
  {
    ...
    body: {
      ...
      es: [
        'Una página es el instrumento: el Observatorio. Un globo con terreno 3D (MapLibre GL, teselas raster-dem terrarium, sombreado, la imagen Sentinel-2 sin nubes de EOX y etiquetas OpenFreeMap) sostiene la ventana del sitio, los polígonos de referencia, el cuadro anual drapeado sobre el relieve, la máscara cocinada del año, la diferencia de DEM y el raster en vivo. El riel elige el sitio y muestra su ficha; la línea de tiempo reproduce los años (en pausa por defecto, teclado, marcas, sensor y fecha, banderas); el panel de instrumento tiene Mirar (compuestos, índices), Encontrar (Otsu, k-means, SAM, bosque aleatorio, U-Net) y Relieve (época, diferencia, perfil); el cajón de series grafica la serie de área minada con quiebres y alarmas.',
        'Las otras páginas son documentación que lee los mismos artefactos: Atlas (la tabla del catálogo), Métodos (los doce métodos con ecuaciones KaTeX, DOI y el benchmark retenido desde models/benchmark.json), Datos (fuentes, licencias, atribución, los dos contratos) y Acerca de. EN y ES, claro y oscuro, sin destello del tema equivocado; la URL lleva el sitio (?site=).',
        'Construcción: copy-data.mjs superpone data/derived, models/ y el runtime de onnxruntime-web en public/; contract.ts refleja el contrato de artefacto y controla cada lectura; scripts/check_artifacts.py verifica bytes y sha256 en disco y al desplegar; deploy/deploy.ps1 construye, envia por ssh a un directorio de release, cambia la raiz web de forma atomica, recarga nginx y luego verifica el título en vivo, el catálogo y un enlace profundo. CI corre las pruebas del pipeline, el typecheck del frontend, las pruebas unitarias y el build, y las guardas del repositorio.',
      ],
    },
  },
  {
    ...
    body: {
      ...
      es: [
        'Cuatro preguntas, doce métodos, cada uno con su ecuación y su fuente en la página Métodos. Qué estoy mirando: reflectancia de superficie desde los números digitales de nivel 2 (escala y desplazamiento por archivo), compuestos con un estiramiento impreso, índices de diferencia normalizada con pisos de denominador, y tres razones de grupos minerales que la app llama indicadores, nunca mapas minerales.',
        'Dónde está la mina: Otsu sobre el índice de suelo desnudo con pruebas de vegetación y agua, k-means++ sobre los espectros estandarizados, el ángulo espectral contra los polígonos de referencia; luego el par aprendido, un bosque aleatorio acotado sobre dieciséis atributos por píxel y una U-Net de 7,85 M de parámetros sobre las seis bandas, ambos entrenados con Jasansky et al. 2024 con todos los sitios del catálogo retenidos, ambos exportados a ONNX con compuertas de paridad, ambos evaluados sobre las mismas teselas retenidas que los tres clásicos.',
        'Cómo cambió: vectores de cambio entre dos fechas en vivo; CUSUM y PELT (verificado contra ruptures) sobre la serie anual de área minada dentro de la envolvente; una regresión armónica con quiebres sobre la serie densa Sentinel-2. Cuánta roca se movió: el DEM Copernicus menos SRTM sobre la grilla de 30 m, corregido por geoide y sin sesgo sobre terreno estable, con un piso de ruido medido y los volúmenes de corte y relleno más allá de el.',
      ],
    },
  },
  {
    ...
    body: {
      ...
      es: [
        'Contrato 1, ingesta: data/examples/sites.json declara cada sitio (id, nombres, país ISO3, categorías, semilla, ventana en km, primer año, meses de temporada, hechos con URL de fuente); la etapa catalog rechaza una semilla a más de 3 km de un polígono de referencia, una ventana fuera de 4 a 40 km, un hecho sin fuente, y escribe site.json (la grilla UTM: un ancho múltiplo de tres para que la grilla Landsat de 30 m anide en la de 10 m) y polygons.geojson.',
        'Contrato 2, artefacto: cada etapa escribe un JSON lateral (scenes, frames con huecos, masks, series, dense, dem) y la etapa export arma manifest.json por sitio con bytes y sha256 de cada archivo y catalog.json; validate relee todo y rechaza un archivo faltante, un hash desviado, un año sin cuadro ni razón registrada, una serie que nombra un año sin cuadro, o dos versiones del motor en un árbol. Los artefactos de texto son LF en toda plataforma y se hashean tal como se guardan.',
        'El mismo contrato se refleja en contract.ts, así la web lee solo lo que los manifiestos declaran; la guarda de artefactos corre en CI y antes de cada despliegue; los modelos llevan su propio registro (datos de entrenamiento, partición, semilla, puntajes retenidos, paridad, sha256). Traiga su propio sitio: agreguelo a sites.json, corra la cocina, y cada página de la app lo sigue.',
      ],
    },
  },
];
