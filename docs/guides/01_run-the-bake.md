...
| `dense` | `site.json`, `polygons.geojson`; Earth Search for every Sentinel-2 date since 2017 | `sites/<id>/dense.json`: the envelope mean of BSI (and NDVI, MNDWI) per clear date at 60 m, rejected dates with the reason; resumable. The file carries `status`: `partial` while the walk checkpoints every twenty kept dates, `complete` after the last candidate date; the series stage folds a complete walk only, and the build overlay never ships this file (the app reads the dense block inside `series.json`) |
