...
| frames/*.webp, masks/*.png, series.json, dem/*.png, terrain/{z}/{x}/{y}.png}  /models/*.{onnx,bin} |
+------------------------------------------------------------------------------------------------+
...
LIVE B   the worker walks the exported random forest (flat node arrays, TypeScript traversal) and
         onnxruntime-web runs the U-Net on the same window; both paint the mine-footprint probability
         as an overlay.
...
| learned models | `train/fetch_tiles.py`, `train/common.py`, `train/train_rf.py`, `train/unet_model.py`, `train/train_unet.py`, `train/export_unet.py`, `train/evaluate.py`, `train/export_forest.py`, `train/forest_format.py`, `train/make_golden.py`; `models/registry.json` | `workers/features.ts` (golden fixture), `workers/forest.ts` (the forest traversal, golden parity), `workers/onnx.ts` (onnxruntime-web, the U-Net), `state/models.ts` |
